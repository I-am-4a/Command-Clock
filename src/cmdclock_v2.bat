...
cls
pushd %0\..
title CommandClock v2
set tlag=0
set StartedAt=%date%_%time%
set version_def=CommandClock-v2

set confname=cmdclock.cfg_v2
...
if not exist %confname% (
	...
	echo version=CommandClock-v2>> %confname%
)
...
if %debug% == true title CommandClock v2 [Debug=on] TimeLag=%nlag%00ms TotalLag=%tlag%00ms TimeAlign=%timealign% StartedAt=%StartedAt%
